...
> 소스: `usecases/extraction.py`, `main.py`의 `extract_all_for_page()`
...
> 소스: `usecases/merging.py` → `processors/llm_merger.py`, `prompts.py`
...
> 소스: `usecases/finalizing.py` → `processors/final_orchestrator.py`
...
| 파일 | 역할 |
|------|------|
| `usecases/models.py` | I/O 데이터클래스 (PageInput, ExtractionResult 등) |
| `usecases/extraction.py` | Stage 1: 4개 추출 유스케이스 함수 |
| `usecases/merging.py` | Stage 2: 페이지별 병합 유스케이스 |
| `usecases/finalizing.py` | Stage 3: 최종 문서 생성 유스케이스 |
| `processors/llm_merger.py` | LLM 병합 로직 (모듈 레벨 함수) |
