...
class DeepAnalysisPipeline:
        ...
        try:
            ...
            page_result = {
                ...
                'successful_extractors': sum(
                    1 for data in extractor_results.values()
                    if not data['result'].get('error')
                )
            }

            self._save_json(page_result, "page_result.json", page_dir)
            if page_result.get('content'):
                self._save_text(page_result['content'], "page_content.txt", page_dir)
            ...
            summary = {
                ...
                'extractors': {
                    name: {
                        ...
                        'text_length': data.get('text_length', data.get('hyperlink_count', 0)),
    ...
    async def process_pdf(self, pdf_path: str) -> None:
        ...
        print(f"    - pymupdf_result.json - PyMuPDF hyperlink extraction")
        print(f"    - pdfplumber_result.json/txt - PDFPlumber extraction")
        print(f"    - clova_ocr_result.json/txt - CLOVA OCR extraction")
        ...
        print(f"    - page_result.json/txt - Final page result")
